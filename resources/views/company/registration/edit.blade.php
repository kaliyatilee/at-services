<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <x-navbars.sidebar activePage="company_registration_edit"></x-navbars.sidebar>
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        <x-navbars.navs.auth titlePage="Edit Company Registration"></x-navbars.navs.auth>
        <!-- End Navbar -->
        <div class="container-fluid px-2 px-md-4">
            ...
            <div class="card card-body mx-3 mx-md-4 mt-n6">
                <div class="card card-plain h-100">
					...
					<form id="edit_company_registration_form" method='POST' action='{{ route('api_update_company_registration', $company_registration->id) }}'>
                            @csrf
                            <div class="row">
                                <div class="mb-3 col-md-6">
                                    <label class="form-label">Select Supplier</label>
                                    <select class="form-control border border-2 p-2" name="supplier">
                                        @foreach($company_registration_suppliers as $company_registration_supplier)
                                            <option value="{{ $company_registration_supplier->id }}">{{ $company_registration_supplier->name }} - {{ $company_registration_supplier->location }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <input type="text" name="name" class="form-control border border-2 p-2"
                                           value="{{ $company_registration->name }}">
										   <input type="hidden" name="id" value="{{ $company_registration->id }}">
                                </div>

                                <div class="mb-3 col-md-6">
                                    <label class="form-label">Phone Number</label>
                                    <input type="text" name="phone" class="form-control border border-2 p-2"
                                           value="{{ $company_registration->phone }}">
                                </div>

                                <div class="mb-3 col-md-6">
                                    <label class="form-label">Charge</label>
                                    <input type="number" name="charge" class="form-control border border-2 p-2" value="{{ $company_registration->charge }}">
								...
								<div class="mb-3 col-md-6">
                                    ...
                                    <input type="number" name="amount_paid" class="form-control border border-2 p-2"
                                           value="{{ $company_registration->amount_paid }}">
                                </div>
								<div class="mb-3 col-md-6">
									<label class="form-label">Transaction Date</label>
									<input type="date" name="transaction_date" class="form-control border border-2 p-2"
									value='{{ isset($company_registration->transaction_date) ? \Carbon\Carbon::parse($company_registration->transaction_date)->format("Y-m-d") : "" }}'>
								</div>

                                <div class="mb-3 col-md-6">
                                    <label class="form-label">Notes</label>
                                    <input type="text" name="notes" class="form-control border border-2 p-2"
                                           value="{{ $company_registration->notes }}">
                                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#edit_company_registration_form').submit(function (e) {
