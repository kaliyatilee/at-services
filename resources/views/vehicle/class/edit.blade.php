<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        <x-navbars.navs.auth titlePage="Edit Vehicle Class"></x-navbars.navs.auth>
        <!-- End Navbar -->
            ...
            <div class="card card-body mx-3 mx-md-4 mt-n6">
                <div class="card card-plain h-100">
                    <div class="card-body p-3">
                        <form id="edit_vehicle_class_form" method='POST' action='{{ route('api_update_vehicle_class', $vehicle_class->id) }}'>
                            @csrf
                            <div class="row">
                                <div class="mb-3 col-md-6">
                                    <label class="form-label">Name</label>
                                    <input type="text" name="name" class="form-control border border-2 p-2"
                                           value='{{ $vehicle_class->name }}'>
                                </div>
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <select class="form-control border border-2 p-2" name="currency_id">
                                        @foreach($currencies as $currency)
                                            <option value="{{ $currency->id }}">{{ $currency->name }} ({{ $currency->exchange_rate }})</option>
                                        @endforeach
                                    </select>
                                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#edit_vehicle_class_form').submit(function (e) {
